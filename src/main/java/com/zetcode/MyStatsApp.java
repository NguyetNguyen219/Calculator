...
public class MyStatsApp {
    ...
    public void run(String[] args) throws IOException {
        ...
        if (line.hasOption("filename")) {
            ...
            if(NumberChecker.checkNumbers(data)) {
                calculateAndPrintStats(data);
            }
            else {
                System.err.println("Warning: The numbers should be <= 20 and >= -20");
            }
        } else {
            printAppHelp();
        }
     ...
     */
    private void calculateAndPrintStats(double[] data) throws IOException {
        BufferedWriter writer = new BufferedWriter(new FileWriter("src/main/resources/dataOutput.txt"));

        writer.write(new Date() + "\n\n");
        writer.write(String.format("Geometric mean: %f%n", StatUtils.geometricMean(data)));
        writer.write(String.format("Arithmetic mean: %f%n", StatUtils.mean(data)));
        writer.write(String.format("Max: %f%n", StatUtils.max(data)));
        writer.write(String.format("Min: %f%n", StatUtils.min(data)));
        writer.write(String.format("Sum: %f%n", StatUtils.sum(data)));
        writer.write(String.format("Variance: %f%n", StatUtils.variance(data)));
